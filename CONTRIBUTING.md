...
- **Node.js LTS** - [Download](https://nodejs.org/) | [Homebrew](https://formulae.brew.sh/formula/node)
...
1. Fork and clone the repository:
   ```bash
   git clone https://github.com/YOUR-USERNAME/caldova-careers.git
   cd caldova-careers
   ```
...
2. Install dependencies:
   ```bash
   npm install
   npm run test:e2e:install   # install Chromium, only needed for the E2E tests
   ```
...
`predev` runs `db:setup` (applies the `applications` migration) before starting the dev server.

## Project Structure

- `src/content/jobs/` - job postings (Markdown + frontmatter)
- `src/content.config.ts` - the `jobs` content collection schema
- `src/lib/` - pure helpers over `Job[]` and the application validation/insert helpers
...
### Data Layer (content collections + Drizzle)

- Job postings are Markdown files in `src/content/jobs/`, not database rows. Do not add jobs to the database or create seed data.
- Applications are the only database-backed feature. Define tables in `db/schema.ts`; generate a migration with `npm run db:generate` after schema changes.
- Use explicit type hints for all exported function parameters and return values.
- Keep data-access helpers in `src/lib/` with an injectable `db` argument so they stay unit-testable.
- Add or update Vitest tests for any data-layer change and run them before submitting: `npm run test:unit`
   - All tests must pass

### Frontend (Astro)

- Build UI as `.astro` pages and components; query data in frontmatter (static output).
- Follow the dark slate Caldova theme using Tailwind CSS utility classes.
- Prefer native HTML, add visible focus states, and add `data-testid` attributes to interactive elements for testing.
...
3. Run the checks to ensure nothing is broken:
   ```bash
   npm run lint
   npm run typecheck
   npm run test:unit
...
- Use the pull request template, and ensure all sections are completed.
...
Found a bug or have a feature request? Please [open an issue](https://github.com/github-samples/caldova-careers/issues/new) with:
